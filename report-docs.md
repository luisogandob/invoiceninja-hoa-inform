...
- **Cargos Emitidos en el Período**: Suma de todas las facturas emitidas a residentes durante el período (cuotas de mantenimiento u otros cargos).
- **Pagos Recibidos en el Período**: Total de pagos registrados durante el período.
...
Mapa de calor (heatmap) que muestra el historial de pago de cada unidad de vivienda mes a mes durante los últimos meses. Cada celda representa el estado del pago de las facturas emitidas en ese mes para esa unidad.
...
Detalla las cuentas pendientes de cobro al cierre del período.

- **CxC por Grupo de Cliente**: Distribución del saldo entre los grupos de residentes definidos (ej. por edificio o sector).
- **Saldo por Antigüedad de Emisión**: Clasifica el saldo según hace cuántos días fue emitida la factura:
...
Presenta todos los movimientos de efectivo del período en orden cronológico:

- **Entradas** (verde, flecha arriba): Pagos recibidos de residentes.
- **Salidas** (rojo, flecha abajo): Gastos pagados durante el período.
- **Resultado del Período**: Diferencia entre el total de entradas y el total de salidas. Verde = superávit, Rojo = déficit.
- **Balance en Banco**: Estimado del saldo bancario acumulado al cierre del período, pendiente de conciliación con el estado de cuenta bancario real.
