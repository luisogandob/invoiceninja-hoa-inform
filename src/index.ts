...
This report was automatically generated by the HOA Financial Reporting System.
  ...
  private generateEmailHtml(
    ...
    const unpaidRows = unpaidInvoices.slice(0, 10) // Show top 10 unpaid
      .map((invoice) => {
...
<body>
  <div class="container">
    ...
    ${unpaidInvoices.length > 10 ? `<p><em>Showing top 10 of ${unpaidInvoices.length} unpaid invoices. See PDF for complete list.</em></p>` : ''}
    ` : ''}
  ...
  async testInform(options: ReportOptions = {}): Promise<ReportResult> {
    ...
    try {
      ...
      if (unpaidInvoices.length > 0) {
        ...
        unpaidInvoices.slice(0, 10).forEach((invoice) => {
          const balance = parseFloat(String(invoice.balance || 0));
          const client = invoice.client_name || invoice.client?.name || 'Unknown';
          const number = invoice.number || '-';
        ...
        if (unpaidInvoices.length > 10) {
          console.log(`   ... and ${unpaidInvoices.length - 10} more`);
        }
        console.log('');
      }
