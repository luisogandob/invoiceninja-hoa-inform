...
class HOAInformAutomation {
  ...
  /** @deprecated Use hoaReportGenerator instead */
...
<body>
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // NEW HOA REPORT  (refactored)
  // ═══════════════════════════════════════════════════════════════════════════

  /**
   * Generate the new HOA income report as a PDF and optionally send it by email.
   *
   * Report sections:
   ...
   */
  async generateHoaReport(options: ReportOptions = {}): Promise<ReportResult> {
    const {
      period = (process.env.REPORT_PERIOD as PeriodType) || 'current-month',
      customRange = null,
      emailTo = null,
    ...
    try {
      console.log('Starting HOA Report Generation (new format)...');
      ...
      return {
        success: true,
        message: 'HOA report generated and sent successfully',
        stats: {
          expenseCount: 0,
          incomeCount: filteredPeriodInvoices.length,
          paymentCount: filteredPeriodPayments.length,
          unpaidInvoiceCount: allInvoices.filter(inv => parseFloat(String(inv.balance || 0)) > 0).length,
          totalExpenses: 0,
          ...
          netAmount: reportData.totalPaymentsInPeriod - reportData.totalInvoicedInPeriod,
          period: periodString
        }
      };
    } catch (error) {
      console.error('Error generating HOA report:', (error as Error).message);
      throw error;
    }
  }

  /**
   * Preview the new HOA report: print KPIs to console and save PDF to disk
   * without sending email.
   */
  async testHoaReport(options: ReportOptions = {}): Promise<ReportResult> {
    const {
      period = (process.env.REPORT_PERIOD as PeriodType) || 'current-month',
      customRange = null,
    ...
    try {
      console.log('═══════════════════════════════════════════════════════════');
      console.log('TEST HOA REPORT (new format) — No Email');
      console.log('═══════════════════════════════════════════════════════════\n');
      ...
      const periodString = formatPeriodString(period, dateRange);
      console.log('═══════════════════════════════════════════════════════════');
      console.log('✓ TEST HOA REPORT COMPLETED');
      console.log('═══════════════════════════════════════════════════════════\n');

      return {
        success: true,
        message: 'Test HOA report completed successfully',
        stats: {
          expenseCount: 0,
          incomeCount: filteredPeriodInvoices.length,
          paymentCount: filteredPeriodPayments.length,
      ...
      console.error('\n✗ Error during test HOA report:', (error as Error).message);
      throw error;
    }
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    if (command === 'test') {
    ...
    } else if (command === 'hoa-report') {
      // New HOA report — generate and send
      const period = (args[1] as PeriodType) || (process.env.REPORT_PERIOD as PeriodType) || 'current-month';
      const result = await automation.generateHoaReport({
        period: period,
        saveToFile: true,
        outputPath: './hoa-report.pdf'
      });
      if (result.stats) {
        console.log('\n✓ HOA report generated and sent!');
        ...
        console.log(`  CxC al final:     $${result.stats.totalUnpaidBalance.toFixed(2)}`);
      }
    } else if (command === 'test-hoa-report') {
      // New HOA report — preview without email
      const period = (args[1] as PeriodType) || (process.env.REPORT_PERIOD as PeriodType) || 'current-month';
      const result = await automation.testHoaReport({
        period: period,
        outputPath: './hoa-report-test.pdf'
      });
      if (result.stats) {
        console.log('Resumen HOA Report:');
        console.log(`  Período: ${result.stats.period}`);
        console.log(`  Cuotas emitidas:  $${result.stats.totalIncome.toFixed(2)}`);
        console.log(`  Pagos recibidos:  $${result.stats.totalPayments.toFixed(2)}`);
        console.log(`  CxC al final:     $${result.stats.totalUnpaidBalance.toFixed(2)}`);
      ...
      console.log('  npm start test-hoa-report [period]  - Preview new HOA report (no email)');
      console.log('  npm start hoa-report [period]       - Generate and send new HOA report');
      console.log('  npm start test-inform [period]      - (deprecated) Preview old report');
      console.log('  npm start report [period]           - (deprecated) Generate and send old report');
